<div class="w-100">
    ...
    <div class="bg-white shadow-md dark:bg-gray-800 pt-3 sm:rounded-lg">
        <livewire:search-bar.search-bar
            {{-- REQUIRED --}} model="App\Models\Model\Times"
            {{-- REQUIRED --}} modelId="times.id"
            {{-- REQUIRED --}} columnsInclude="day,athletes.name,modalities.title,record,pool,distance,type_time"
            {{-- REQUIRED --}} columnsNames="Data,Atleta,Modalidade,Tempo,Piscina,Distância,Tipo"
            {{-- REQUIRED --}} searchable="type_time,athletes.name,modalities.title"

            searchableDates="day"
            {{-- OK --}} relationTables="athletes,athletes.id,times.athlete_id | modalities,modalities.id,times.modality_id "
            {{-- OK --}} showButtons="Ações"
            {{-- OK --}} sort="times.day , asc | times.record , asc"
            {{-- OK --}} paginate="15"
        />
    </div>
    {{-- MODAL DELETE --}}
